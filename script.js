...
/* Modern and elegant header with geometric pattern - reduced height */
header.compact-header {
  text-align: center;
  padding: 1.2rem 0 0.8rem;
  background: linear-gradient(135deg, #1a1a1a 0%, #2c3e50 100%);
  box-shadow: 0 4px 20px rgba(0,0,0,0.1);
  margin-bottom: 1.5rem; /* Standard spacing after header */
  position: relative;
  overflow: hidden;
  color: #fff;
}
...
.content-layout {
  ...
  gap: 1.5rem; /* Consistent spacing between major content blocks */
}

/* Standard section styling - uniform spacing */
.standard-section {
  background: #fff;
  padding: 1.2rem;
  border-radius: 8px;
...
.asset-chart-area.visible {
  height: 300px; /* Increased height for better year label visibility */
  max-width: 90%; /* Reduced width of charts */
...
.events-grid {
  ...
  gap: 0.8rem;
  width: 100%;
  padding-bottom: 1rem; /* Added padding to bottom of events grid */
  border-bottom: 1px solid #f0f0f0; /* Added border to bottom of events grid */
}
...
.event-item {
  ...
  margin-bottom: 0.5rem; /* Added margin to bottom of event items */
}
...
.scarcity-comparison {
  display: flex;
  flex-direction: row; /* Force items to be in a row */
...
/* Visualization of mined bitcoins - simplified */
.supply-progress {
  height: 24px;
  background: #f0f0f0;
  border-radius: 4px;
...
.supply-progress-fill:before {
  ...
  height: 100%;
  width: 92.23%;
  background: rgba(247, 147, 26, 0.15); /* Very light Bitcoin orange */
  border-right: 1px solid rgba(247, 147, 26, 0.5); /* Subtle border */
}
